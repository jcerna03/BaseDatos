...
$b = array_keys($tablas);
  ...
  function getEventos($user_id){
    $sql = "SELECT id, titulo as `title`, fecha_inicio as `start_date`, hora_inicio as `start_hour`, fecha_fin as `end_date`, hora_fin as `end_hour`, completo as `allDay` FROM eventos WHERE id_usuario = ".$user_id.";";
    return $this->ejecutarQuery($sql);
  
  }
  
  function getId($tabla){
